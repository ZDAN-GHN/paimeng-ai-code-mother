...
- 实施票据链：~~#3 骨架~~ → ~~#4 run 生命周期~~ → ~~#5 最小生成流 + 契约定稿~~ → ~~#6 回调打通~~ → ~~#7 需求工程（访谈/线框/确认闸门）~~ → #8-#9 核心能力 → #10 积分 → #11 对账 → #12/#13 前端 → #14 灰度+T21。
...
- #7：已完成 需求工程（访谈/线框/确认 + codegen 闸门），`docs/ts_agent/contract.md` 已增补新端点与闸门；下一步 #8 生成核心移植（AI 工具四件 + 代码块解析 + Guardrail）。
