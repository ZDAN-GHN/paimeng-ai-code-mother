...
## 当前状态（**P3 收官 + 架构优雅化第一批 6/6 收口（#16–#21 全部完成）**，2026-09-08；TS Agent 直连链路为唯一生成实现）
...
- 配置键：`PEXELS_API_KEY`/`DASHSCOPE_API_KEY`/`IMAGE_MODEL`/`MODEL_FAST|STANDARD|DEEP`（.env.example 已加）；真实渠道为 `ZHIPU_API_KEY` 与 `DASHSCOPE_CODING_API_KEY`。
- **模型选型与真实 provider 接入（2026-09-12 已更新）**：路由档（`MODEL_ROUTER`，调用点为后续特性）= 智谱 `glm-4-flash-250414`；快速档（`MODEL_FAST`）= 智谱 `glm-4.7-flash`（质检档 scripted-quality 复用快速模型）；标准档（`MODEL_STANDARD`）= **qwen3.6-plus @ DashScope Coding**；深度档（`MODEL_DEEP`）= **qwen3.7-plus @ DashScope Coding**。真实 provider 实现位于 `src/llm/real.ts`：智谱与 DashScope Coding 两渠道 OpenAI 兼容 API 经 `customProvider` 统一封装；路由层按配置装配——**两渠道密钥须齐全（缺一 fail-fast 拒启，部分配置=启动失败而非渠道级降级）**，全空回退假 LLM；provider 注入类型 `LlmProvider = Pick<ReturnType<typeof customProvider>,'languageModel'>`。
...
- **架构优雅化第一批（父 issue #15）6/6 收口（2026-09-08）**：#16/#17/#18/#19/#20/#21 全部完成。#21 弹药已消费大半（httpError 单点/双轨预检/编排归位/script 退场），未消费残余：三个 body schema 的 runId/appId/userId 字段形状重复 ×3 可提取共享 shape、stream handler error 帧同形可折局部 fail()——留作后续判断题。审查根因结论在 #15 正文；**XState「图只簿记不驱动控制流」双轨决策待用户拍板（#22，关键变量 = P2 断点续传的快照序列化预期）**。
