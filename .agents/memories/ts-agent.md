...
- 实施票据链：~~#3 骨架~~ → ~~#4 run 生命周期~~ → ~~#5 最小生成流 + 契约定稿~~ → ~~#6 回调打通~~ → ~~#7 需求工程（访谈/线框/确认闸门）~~ → ~~#8 生成核心移植（工具四件+解析+Guardrail）~~ → ~~#12 前端通道切换~~ → #9 核心能力（续） → #10 积分 → #11 对账 → #13 前端功能补齐 → #14 灰度+T21。
...
- #7：已完成 需求工程（访谈/线框/确认 + codegen 闸门），`docs/ts_agent/contract.md` 已增补新端点与闸门。
- #8：已完成 生成核心移植（Guardrail + 代码块解析 + 文件/图片工具 + 提示词 7 份 + 导览组件），详见「当前状态」。
