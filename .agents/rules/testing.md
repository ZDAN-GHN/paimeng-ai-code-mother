...
- Vue 前端：在 `paimeng-ai-code-mother-frontend/` 执行 `npm run type-check`、`npm run lint`、`npm run build`；类型生成使用 `npm run openapi2ts`。
- Java：使用仓库 `./mvnw`，JDK 21；按改动范围执行 compile/test。
