...
description: 启动并排查本项目的本地开发环境，覆盖 MySQL、Redis、PostgreSQL、Java Spring Boot、Python Agent 和 Vue 前端；当用户要求启动项目、运行本地全栈、诊断启动失败或验证双后端联调时手动调用，未经授权不修改数据、覆盖敏感配置或执行破坏性操作。
user-invocable: true
...
仅用于本仓库的本地开发启动、启动前检查、进程/端口冲突诊断、配置与依赖排查、Java-Python-前端联调验证。

不用于生产发布、容器编排、数据库结构变更、删除旧链路、修改业务代码或自动修复未知故障。遇到这些需求，应停止并让用户明确授权或切换到专门流程。

默认工作目录是仓库根目录：`/mnt/c/Users/LXH/IdeaProjects/paimeng-ai-code-mother`。所有命令优先从仓库根目录执行；Python 命令进入 `paimeng-ai-code-agent/`，前端命令进入 `paimeng-ai-code-mother-frontend/`。
...
- PostgreSQL：WSL 用户态 PG16 @ 5432（仅 LangGraph checkpoint）。
...
- **Java-only**：MySQL + Redis + Java。适合业务接口和旧 Java AI 链路，Python Agent 可不启动。
- **Python 灰度全栈**：MySQL + Redis + PostgreSQL + Python Agent + Java（`python-agent.enabled=true`）+ 前端。适合验证新链路。
- **前端联调**：先检查 Java 是否已运行，再启动前端；前端只访问 Java，不直接访问 Python。
...
检查仓库状态、Java/Python/Node/uv/Maven 可执行文件和版本；只输出版本与路径，不输出环境变量值。确认关键文件存在：
...
```bash
ss -ltnp | rg ':(3306|5432|6379|8090|8123|5173)\\b' || true
```
...
- Python：`paimeng-ai-code-agent/.env` 必须存在；以 `.env.example` 为模板确认 `PYTHON_AGENT_TOKEN`、`WORKSPACE_ROOT`、`DATABASE_URL`、`JAVA_BASE_URL`、模型 API 配置已非空。用 shell 判断变量是否为空，不 `cat` 整个 `.env`。
- 前端：`paimeng-ai-code-mother-frontend/.env.development` 必须存在；确认 API 基础地址指向 Java。
- Python 与 Java 的 Bearer token 必须相同；共享工作区必须都指向同一绝对路径，并位于仓库的 `tmp/code_output` 体系内。
...
密码不得写入命令行、日志或回复；需要密码时使用已有安全环境、交互输入或让用户自行执行。MySQL 数据库不存在或表未初始化时，只报告事实并提示 `sql/create_table.sql`，不得自动执行建库脚本；**仅当用户明确授权导入时**才执行，且导入前先 `SHOW TABLES` 检查目标库（脚本中 `create table app` 等语句非幂等，重复导入会报错）。PostgreSQL 仅用于 Python LangGraph checkpoint，不要把 MySQL 业务表迁移到 PostgreSQL。
...
# 迁移前 venv 在 .venv；迁入 wsl-rt-env/python/.venv 后改 UV_PROJECT_ENVIRONMENT 为该路径
...
Python 使用仓库约定的虚拟环境和 `uv.lock`，不要使用全局 pip；可用 `uv pip check` 验证依赖。虚拟环境新约定为 `wsl-rt-env/python/.venv`（迁移前实际在 `paimeng-ai-code-agent/.venv`）。Java 编译要求 JDK 21：
...
# Python Agent（全栈模式）
cd paimeng-ai-code-agent
UV_PROJECT_ENVIRONMENT=.venv uv run uvicorn app.main:app --port 8090   # 迁入 wsl-rt-env/python/.venv 后改路径
...
- **Python**：DSH 沙箱 workspace-write 下 `uv run` 因 `~/.cache/uv` 写入被拒（exit 2）。虚拟环境已同步时直接调用：
  `cd paimeng-ai-code-agent && .venv/bin/uvicorn app.main:app --port 8090`（迁入 `wsl-rt-env/python/.venv` 后为 `wsl-rt-env/python/.venv/bin/uvicorn`）；仅在 venv 缺失/过期时才需提权跑 `uv sync`。
...
推荐顺序是 Python → Java → 前端；Java-only 跳过 Python。若服务已健康运行，不重复启动。记录每个 PID、端口、日志文件和启动命令；不要将 token、API key 或完整配置写入日志报告。

完成标准：

- Python：`curl -fsS http://localhost:8090/healthz` 返回 HTTP 200 且 JSON 含 `"status":"ok"`。
...
全栈模式下逐层验证，不因 Python `/healthz` 成功就认为链路可用：

1. Python `/healthz` 成功。
2. Java 能访问 Python 健康地址，且 Java 日志无连接超时、401 或 5xx。
3. Java 能连接 MySQL、Redis；Python 能连接 PostgreSQL checkpoint 数据库。
4. 前端打开后只请求 Java API；不要把 Python 地址配置给浏览器。
5. 需要验证代码生成时，使用测试账号/测试应用和最小无敏感 prompt；确认 SSE 有数据、结束事件和错误事件语义正常。不要用真实密钥、生产数据或会触发部署的请求。

灰度模式验证 `python-agent.enabled=true`；回滚验证只在用户要求时将其恢复为原值。契约或回归验证按权威文档执行：`docs/py_agent/task_plan.md` §1、§5、§9，以及 `docs/py_agent/sse_baseline.py`。
...
| Python 启动失败 | `.python-version`、虚拟环境（新约定 `wsl-rt-env/python/.venv`，迁移前 `paimeng-ai-code-agent/.venv`）、`uv sync`、配置键名 | 不使用全局 pip，不打印 `.env` |
| `/healthz` 失败 | Python 日志、8090 监听状态、导入异常 | 修复环境/配置前不测 SSE |
| Java 401/连接 Python 失败 | base URL、token 是否一致、超时配置 | 只核对存在性和脱敏摘要 |
| 数据库连接失败 | 服务状态、地址/端口、数据库名、凭据来源 | 不重置密码、不删库、不自动导入 |
| 前端白屏或 API 失败 | dev server 日志、浏览器 Network、Java 地址 | 不直接改前端 SSE 协议 |
| SSE 卡住/事件不一致 | Java/Python 两侧日志、超时、契约文档 | 先保存最小复现与事件类型，再修改 |
...
| uv/npm 报 `Permission denied`（`~/.cache`、`~/.npm`） | 是否处于 DSH 沙箱 workspace-write 模式 | 改用 `.venv/bin/uvicorn` 直调、npm `--cache` 指向仓库 `tmp/`；确需写入时走提权授权 |
...
- 不把前端直接连到 Python；不让 Python 直接访问 MySQL 业务库；不把 MySQL 业务表迁到 PostgreSQL。
...
- [ ] 目标模式所需 MySQL、Redis、PostgreSQL 均已探活，或明确记录阻塞原因。
- [ ] Python `/healthz`、Java 端口/API 文档、前端 URL 均有实际证据。
- [ ] 全栈模式已确认 Java→Python 鉴权、数据库连接、共享工作区和前端→Java 路径。
...
```text
模式：Java-only / Python 灰度全栈 / 前端联调
状态：通过 / 部分通过 / 阻塞
服务：MySQL、Redis、PostgreSQL、Python、Java、前端（逐项写端口和探活结果）
验证：已执行的命令或接口（脱敏）及结果
问题：现象 -> 证据 -> 判断 -> 处理
阻塞/授权：仍需用户提供或确认的事项
```
