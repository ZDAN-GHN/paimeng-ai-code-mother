...
## Scope
- Active services: Java backend in `src/`, TS Agent in `paimeng-ai-code-agent/`, Vue frontend in `paimeng-ai-code-mother-frontend/`, and P4 RAG work in `paimeng-ai-code-rag/`.
- `paimeng-ai-code-mother-microservice/` is a deprecated experiment; do not use it as a development or migration baseline.

## Toolchains
| Area | Requirement | Dependency command |
|---|---|---|
| Java backend | Java 21, Maven Wrapper | `./mvnw dependency:go-offline` |
| TS Agent | Node.js >= 20, npm lockfile | `cd paimeng-ai-code-agent && npm install` |
| Frontend | npm lockfile | `cd paimeng-ai-code-mother-frontend && npm install` |
| RAG | Python >= 3.14, uv lockfile | `cd paimeng-ai-code-rag && uv sync` |

## Commands
| Area | Task | Command |
|---|---|---|
| Java backend | Run one test class | `./mvnw -Dtest=ClassNameTest test` |
| Java backend | Verify | `./mvnw verify` |
...
| Frontend | Type-check | `cd paimeng-ai-code-mother-frontend && npm run type-check` |
| Frontend | Lint (writes fixes) | `cd paimeng-ai-code-mother-frontend && npm run lint` |
| Frontend | Build | `cd paimeng-ai-code-mother-frontend && npm run build` |
| Frontend | Regenerate API types | `cd paimeng-ai-code-mother-frontend && npm run openapi2ts` |
...
- Keep browser-to-Agent changes coordinated with the frontend SSE client in `paimeng-ai-code-mother-frontend/src/utils/agentSse.ts`.
...
| Frontend run and test details | `paimeng-ai-code-mother-frontend/README.md` |
