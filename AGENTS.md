...
**过渡态**：Python Agent 已定稿**全面退役**（用户决策 2026-09-03）：回切旧 Java AI 链路兜底（`python-agent.enabled=false`，**P0 已执行 2026-09-03**，本地配置已回切并 e2e 验证）、T21 门禁重定向为"TS Agent 契约对等 + 回归全绿"、PG 即刻停用；**旧目录已整体重命名为 `paimeng-ai-code-rag/`（RAG 骨架复用起点，退役代码 P4 精简），TS Agent 落位 `paimeng-ai-code-agent/`（目录名复用）**。`docs/py_agent/` 全目录转为历史参考（提示词/契约/验证记录是 TS 移植参考源）。
...
- **TS Agent (npm)**：在 `paimeng-ai-code-agent/` 目录使用 `npm install`；技术栈 Node + Fastify + Vercel AI SDK + XState v5
- **Python RAG (uv，P4)**：`paimeng-ai-code-rag/`（旧 Python Agent 目录重命名，骨架复用起点）；沿用 uv 锁定依赖约定
...
| 运行 TS Agent | `cd paimeng-ai-code-agent && npm run dev`（端口 8092） |
| 运行全部 TS Agent 测试 | `cd paimeng-ai-code-agent && npm test` |
...
- **TS Agent 约定（`paimeng-ai-code-agent/`）**：编排收敛三纪律——phase 枚举进 `generation_run` DDL、跨请求等待仅显式持久化状态、重试有界；浏览器 wire 用新 SSE 协议（四类事件语义保留 + `milestone` 一等事件），契约落 `docs/ts_agent/contract.md`
