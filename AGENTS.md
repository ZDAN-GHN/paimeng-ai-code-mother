...
- 实现功能、开发需求、编码、写代码、落地已明确的 spec/Ticket/验收标准或已授权修复目标时：自动使用 `realize` 技能，勿手工重复其步骤
...
- 准备向用户提出需求、设计或技术选择问题时，以及明确的设计审查、方案评估、API 契约变更、跨服务调用设计、架构风险检查时：先使用 `grilling` 技能自主消解，勿直接提问或手工重复其步骤
