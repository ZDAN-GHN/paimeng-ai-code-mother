...
本计划不实施产品功能、不选择尚未决定的模板或执行后端、不将旧 `App` / `GenerationRun` 语义直接升级为新平台领域模型，也不创建额外的 Agent SDK 集成。Pi SDK 是唯一允许接入的 Agent Engine；RAG 不进入该主链路。
...
- 对应：`OQ-001`。
- 需要决定：首个 TypeScript 全栈模板、ORM 和 Migration 工具组合，以及该模板的工程验证命令。
- 影响：阻塞 `T-04`、`T-05`、`T-07`。
- 验收：决策记录列出固定版本、许可证/维护状态、模板构建/测试/Migration 命令和兼容 Migration 能力。
...
- 需要决定：删除是逻辑归档/停止公开运行，还是物理删除；删除后 Application、Requirement、Task、Run、SourceRevision、Release、Deployment、数据库和公开 URL 的可观察行为、恢复能力与权限。
- 影响：阻塞 `T-02`、`T-03`，并决定授权删除成功路径、拒绝路径和前端呈现。
- 验收：删除策略写入工程规格与 Issue #65，明确 Owner/System Administrator 权限、非授权拒绝、公开入口处理、证据链/数据保留与恢复或不可恢复边界。
...
**关联规格：** `R-001`、`R-002`、`AC-001` 至 `AC-004`、`CT-001`、`CST-001`。
...
**关联规格：** `R-001`、`R-002`、`AC-001`、`AC-004`、`CT-001`。
...
- [ ] `R-001` 至 `R-007` 的 `AC-001` 至 `AC-025` 都有实际测试或可复现手动证据；`R-007` 包含 Owner 对有效运行、宽限、已停服和已恢复状态的前端可见性证据。
...
| 未定义 Application 删除后的公开入口、证据链和数据处理 | 高 | D-07 先写入工程规格和 Issue #65；任务 2、3 按已批准的删除/归档语义实现授权成功、拒绝和后置行为。 |
| 在 OQ 或 D-06/D-07 未解决时自行选择模板、Sandbox、域名、订阅、状态或删除行为 | 高 | Phase 0 必须由维护者记录决定；相关任务在 `todo.md` 标注为阻塞。 |
...
- [ ] D-01 至 D-04 已在开始对应受阻任务前由维护者决定并记录；D-05 已在任务 6 前记录；D-06、D-07 已写入工程规格与 Issue #65。
...
- 最终状态：`revised; blocked pending D-06 and D-07`
- 任务类型：计划交付。
...
- 审查结论：P2 至 P7 已在计划层消除；P1 与删除数据语义需要维护者决定，已以 D-06/D-07 阻塞相关任务，未擅自补全状态或删除规则。
- 未解决风险：D-01 至 D-04 必须在其对应受阻任务开始前由维护者决定，D-05 必须在任务 6 前记录，D-06/D-07 必须先进入工程规格和 Issue #65；在此之前不得启动相关实现。
