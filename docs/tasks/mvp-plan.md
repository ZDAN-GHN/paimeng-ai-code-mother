...
- 状态：`Approved`。MVP 采用最小托管订阅策略：`activeUntil` 自动到期或运营事件进入宽限后，分别从 `activeUntil` 或 `effectiveAt` 起固定连续 168 小时，时间以 UTC 持久化和比较；宽限结束撤流共享路径并返回 `404`。Platform 内保留有效运行、宽限、已停服和已恢复状态，不发送外部通知。完整边界见 `AD-018`。
- 事件与恢复：订阅事件仅由 System Administrator 或 Platform 内部运营身份经受鉴权的运营命令写入；每个事件有不可变 `eventId` 和 `effectiveAt`，重复事件返回首次结果且不改写状态或截止时间，不同 `eventId` 的有效续费才可更新 `activeUntil`，陈旧事件拒绝。自动到期以 `activeUntil`、运营进入宽限事件以 `effectiveAt` 开始连续 168 小时；定时器重启后追赶已到期撤流。宽限续费立即恢复有效运行，停服续费仅恢复停服前最近一次健康 Deployment 所指向的 Release；归档 Application 的续费事件被拒绝。
- 保留与非范围：MVP 内无限期保留 Application、版本、数据库、日志和审计事实；不接入支付提供商、公开 Webhook、邮件/SMS、退款、订阅自助管理或最终数据删除。
- 已解除：`T-11` 的订阅运营参数决策阻塞；它仍等待任务 9、任务 10。
- 验收：实现必须验证 168 小时期限、幂等/陈旧事件、宽限/停服/恢复四种 Owner 状态、撤流/恢复公开入口和无限期保留；不得把旧 Credit 链路作为订阅事实来源。
...
**关联规格：** `R-007`、`AC-023` 至 `AC-025`、`AD-012`、`AD-018`、`CT-005`、`CST-011`。

**说明：** 按 D-04/`AD-018` 的最小托管策略实现订阅状态事件、168 小时宽限、停止公开 Deployment、数据/证据保留和恢复上一健康 Release；同时提供 Owner/System Administrator 可读取的订阅运行状态 API、OpenAPI 类型和 Vue 界面。界面必须区分有效运行、宽限、已停服和已恢复，且不暴露计费提供商凭据、内部 Deployment 诊断或其他 Application 的状态。该任务不实现支付提供商接入、公开 Webhook、邮件/SMS、退款、最终数据删除、复杂计费或订阅自助管理界面。

**验收标准：**
- [ ] 有效订阅且存在健康 Deployment 时，Application 保持公开运行；Owner 可见“有效运行”状态及当前公开可用性。
- [ ] 自动到期以 `activeUntil`、运营进入宽限事件以 `effectiveAt` 开始连续 168 小时保持公开运行；宽限结束后只停止公开 Deployment 并返回 `404`，定时器重启后追赶已到期撤流，保留 Application、Profile、版本、数据库、日志和记录；Owner 可见“宽限”与“已停服”状态，普通用户无法读取管理状态。
- [ ] 订阅事件仅由 System Administrator 或 Platform 内部运营身份经受鉴权的入口写入，按 `eventId` 幂等且不改写状态或截止时间、拒绝陈旧 `effectiveAt`；宽限续费立即恢复有效运行，停服续费仅恢复停服前最近一次健康 Deployment 所指向的 Release。归档 Application 的续费事件被拒绝；不得重新生成源码或改写 SourceRevision/Profile。Owner 可见“已恢复”及没有健康 Release 时公开不可用的状态。新增状态 API 的前端类型必须由 OpenAPI 生成。
...
| 在剩余 OQ 或 D-05 未解决时自行选择 Snapshot 行为，或绕过 AD-018 接入支付/最终删除 | 高 | Snapshot 对应决策必须由维护者记录；订阅实现只能按已批准的最小托管边界执行。 |
