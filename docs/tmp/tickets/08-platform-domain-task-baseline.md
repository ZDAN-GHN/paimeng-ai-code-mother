...
在 Java Platform Domain 建立 Application、Requirement、Task、Run 与 Trusted Profile 的权威持久模型、受控状态机及版本化 `TaskExecutionBaseline`。
...
只按 D-06 的批准矩阵实现状态边；旧 `App`/`GenerationRun` 不得成为权威写入路径；不得实现 Lease、Sandbox 或完整 Run Context。
