...
> Version: v2；基于 `09-owner-requirement-api.md`，统一 Application 根实体为既有 `app`。

## 目标（Goal）

交付 Owner/System Administrator 基于既有 `app` 聚合根的 Application 创建、读取、归档及不可变 Requirement 接收 API。
...
删除只能按 D-07 已批准的逻辑归档语义实现；非 Owner/System Administrator 必须被拒绝；Application ID 统一使用 `app.id`，不得创建或依赖 `platform_application`；不得在本 Ticket 创建 Task 或 `blocked` 问题。

## 验收标准（Acceptance Criteria）

- [ ] Owner 可创建 Application、提交不可变 Requirement 并读取等待归一化状态。
- [ ] Owner/System Administrator 可按 D-07 归档 Application；公开入口和旧 `/app` 链路不可用，Requirement、审计事实与恢复边界可观察。
