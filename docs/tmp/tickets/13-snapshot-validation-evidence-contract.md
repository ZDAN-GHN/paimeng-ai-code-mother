...
可写 Workspace 不能作为 Validation 或 Release 输入。T-06 只提供验证输入与不可伪造的记录边界，版本晋升由 T-07 负责。
...
`changed` 需要 Diff、Requirement 与理由；`unchanged` 需要理由；`uncertain` 不能验证成功或晋升；不得创建 SourceRevision。
...
- [Snapshot 物理存储选择](05-snapshot-storage-decision.md)
