...
| 质检文件拼接：dist 跳过 + 目录缺失返回空 | test_quality.py ×2 | `test/review.test.ts`（补强既有用例 + 新增 1 例） |
| 十工具名驼峰绑定契约（Java ToolManager 依赖） | test_codegen/test_images 绑定例 | `test/tools-contract.test.ts` |
...
| test_stream_requires_auth | 无 Authorization → 401 | ✅ `test/auth.test.ts`（401 矩阵 8 例） |
| test_stream_with_wrong_token | 错误令牌 → 401 | ✅ `test/auth.test.ts`（乱串/错误密钥） |
...
| 测试 | 语义 | TS 处置 |
|---|---|---|
| test_parse_html_code_extracts_block | 提取 ```html 块 | ✅ `test/parsing.test.ts`（单元能力在档；生成主链路未接线的遗留见 §五-4） |
| test_parse_html_code_fallback_to_whole | 无块时全文兜底 | ✅ `test/parsing.test.ts` |
| test_parse_multi_file_extracts_three_blocks | html/css/js 三块解析 | ✅ `test/parsing.test.ts` |
| test_parse_multi_file_missing_css | 缺块时空串 | ✅ `test/parsing.test.ts` |
| test_to_files_html | 解析结果 → {index.html} | ✅ `test/parsing.test.ts` |
| test_to_files_multi_file_skips_blank | 空白内容不写文件 | ✅ `test/parsing.test.ts` |
...
| test_vue_tools_binding | 六文件工具绑定（驼峰名） | ✅ `test/tools-contract.test.ts`（本票补）+ `fileTools.test.ts` 行为例 |
...
| 测试 | 语义 | TS 处置 |
|---|---|---|
| test_validate_accepts_path_under_root | 根下路径通过 | ✅ `test/workspace.test.ts`（9 例矩阵） |
...
全部 ✅ `test/guardrails.test.ts` 逐条移植：空输入 / 超长（>1000 字）/ 边界 1000 字通过 / 中英敏感词 / 注入模式（忽略指令/扮演管理员/System:/New instructions:）/ 正常提示词通过。拒绝文案与旧实现逐字一致（`stream.test.ts` 断言「输入包含不当内容，请修改后重试」）。
...
| test_image_tools_bound_names | 四工具名对齐 | ✅ `test/tools-contract.test.ts`（本票补） |
