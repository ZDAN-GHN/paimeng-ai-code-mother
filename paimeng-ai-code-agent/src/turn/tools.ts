...
const requestGenerationInputSchema = z.object({
...
async function appendModelEvent(
  ...
  event: {
    kind: 'clarify/asked' | 'wireframe/produced' | 'generation/proposed'
    source: 'model'
...
export function buildSessionTurnTools(context: TurnToolContext) {
  const askUser: TurnTool<z.infer<typeof askUserInputSchema>, AwaitingUserResult> = {
    ...
    async execute(input) {
      ...
      return {
        ...
        reason: 'asked',
        events: [
          { kind: 'clarify/asked', source: 'model', payload: { ...payload, questions, round } },
        ],
      }
    },
  }

  const writeWireframe: TurnTool<
    z.infer<typeof wireframeInputSchema>,
    AwaitingUserResult & { wireframe: { relativeUrl: string; pageCount: number; version: string } }
  > = {
    ...
    async execute(input) {
      ...
      return {
        ...
        reason: 'wireframe',
        wireframe: payload,
        events: [{ kind: 'wireframe/produced', source: 'model', payload }],
      }
    },
  }

  const requestGeneration: TurnTool<
    z.infer<typeof requestGenerationInputSchema>,
    AwaitingUserResult & { approvalId: string }
  > = {
    description:
      '提议开始生成并请求用户审批；不会创建 run、冻结积分、生成代码、构建、部署或修改任意非线框文件。',
  ...
  const readDir: TurnTool<z.infer<typeof readDirInputSchema>, FileToolResult> = {
    description: '只读查看工作区目录结构，不修改工作区。',
  ...
  const searchContentImages: TurnTool<z.infer<typeof searchImagesInputSchema>, ImageToolResult> = {
    description: '只读搜索内容图片，不执行代码生成、构建或部署。',
  ...
  return {
    ask_user: askUser,
    write_wireframe: writeWireframe,
