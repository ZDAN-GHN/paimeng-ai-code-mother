...
describe('local eval observer', () => {
  it('rejects relative paths, regular files, symlink escapes, and production activation', () => {
    ...
    expect(() => LocalEvalObserver.fromEnvSync({ EVAL_OBSERVATION_DIR: 'relative' })).toThrow(
      /absolute/,
    )
    ...
    expect(() =>
      LocalEvalObserver.fromEnvSync({ EVAL_OBSERVATION_DIR: dir, NODE_ENV: 'production' }),
    ).toThrow(/forbidden/)
  })
  ...
  it('writes only bounded safe summaries and uses exclusive output', async () => {
    ...
    await observer.metadata('run-1', {
      model: '{"access_token":"hidden","nested":{"id_token":"secret"}}',
      channel: 'local',
    })
    await observer.callback('run-1', {
      method: 'POST',
      path: '/internal/runs/run-1?api_key=hidden',
      status: 200,
      ok: true,
    })
    await observer.event('run-1', {
      type: 'tool_executed',
      id: 'x',
