...
describe('RunClient evaluation observation', () => {
  ...
  it('reports credit freeze under its actual run id', async () => {
    ...
    const { fetchImpl } = makeMockFetch(() =>
      jsonResponse({ code: 0, data: { runId: 'run-4' }, message: 'ok' }),
    )
    const client = new RunClient({
      baseUrl: 'http://java.invalid/api',
      token: 'test',
      ...
      observer: {
        ...
        metadata: async () => {},
        close: async () => {},
        callback: async (runId, summary) => {
          callbacks.push({ runId, path: summary.path })
        },
      },
    })
    ...
    expect(callbacks).toEqual([
      { runId: 'run-4', path: '/internal/agent/runs/run-4/credit/freeze' },
    ])
  })
})
...
describe('RunClient（generation_run 生命周期）', () => {
  it('createRun：POST /internal/runs，携带 Bearer 与 JSON 体，解析返回 Run', async () => {
    const { fetchImpl, calls } = makeMockFetch(() =>
      jsonResponse({
        code: 0,
        data: { runId: 'run-1', appId: 1, userId: 1, phase: 'interview' },
        message: 'ok',
      }),
    )
  ...
  it('createRun：同 app 并发（409）→ RunConflictError，透传「当前有进行中的任务」文案', async () => {
    ...
    const err = await client
      .createRun({ runId: 'run-2', appId: 1, userId: 1, phase: 'interview' })
      .catch((e) => e)
  ...
  it('内部端点无/错 Bearer（401）→ RunApiError', async () => {
    const { fetchImpl } = makeMockFetch(() =>
      jsonResponse({ code: 40101, data: null, message: '非法调用' }, 401),
    )
    const client = makeClient(fetchImpl)
  ...
  it('updateRun：PATCH /internal/runs/{runId}，runId 编码，仅发送补丁字段', async () => {
    const { fetchImpl, calls } = makeMockFetch(() =>
      jsonResponse({
        code: 0,
        data: { runId: 'run/1', appId: 1, userId: 1, phase: 'coding' },
        message: 'ok',
      }),
    )
    const client = makeClient(fetchImpl)
  ...
  it('getLatestNonTerminalRun：带 userId 附加查询参数', async () => {
    ...
    expect(calls[0]!.url).toBe(
      'http://java:8123/api/internal/apps/7/runs/latest-nonterminal?userId=42',
    )
  })

  it('业务码非 0（HTTP 200）→ RunApiError', async () => {
    const { fetchImpl } = makeMockFetch(() =>
      jsonResponse({ code: 40000, data: null, message: '请求参数错误' }, 200),
    )
    const client = makeClient(fetchImpl)
  ...
  it('freezeCredit：POST /internal/agent/runs/{runId}/credit/freeze，携带 intensity，解析冻结结果', async () => {
    const { fetchImpl, calls } = makeMockFetch(() =>
      jsonResponse({
        code: 0,
        data: { ledgerId: 9, frozenAmount: 100, balance: 400 },
        message: 'ok',
      }),
    )
    const client = makeClient(fetchImpl)
  ...
  it('completeRun：aborted 状态透传 filesWritten（中断折算退款依据）', async () => {
    ...
    await client.completeRun('run-1', {
      ...
      messages: [
        ...
        { messageType: 'ai', content: '生成已中断，已保留 1 个已生成文件' },
      ],
      filesWritten: 1,
    })
