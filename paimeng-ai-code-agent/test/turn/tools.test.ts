...
function makeContext() {
  const batches: Array<{
    events: Array<{ kind: string; source: string; payload: Record<string, unknown> }>
    batchSeq: number
  }> = []
  const files = {
  ...
  const sessionStore: SessionStore = {
    appendBatch: vi.fn(async (input) => {
      ...
      return {
        seqFrom: 1,
        seqTo: input.events.length,
        firstSeqNext: input.events.length + 1,
...
describe('session turn tools', () => {
  it('exposes only free-session and read-only tools', () => {
    ...
    expect(Object.keys(buildSessionTurnTools(context))).not.toEqual(
  ...
  it('asks structured questions and ends the tool turn awaiting the user', async () => {
    ...
    expect(result.events[0]).toMatchObject({
      kind: 'clarify/asked',
      payload: { itemKeys: ['audience'], questions, round: 1 },
    })
    expect(batches[0]).toMatchObject({
      events: [{ kind: 'clarify/asked', source: 'model', payload: { itemKeys: ['audience'] } }],
    })
  })
  ...
  it('writes only a sandboxed wireframe and returns the public wireframe fields', async () => {
    ...
    const result = await buildSessionTurnTools(context).write_wireframe.execute!({
      relativeUrl: 'wireframe/wireframe.html',
      pageCount: 2,
      version: 'v1',
      content: '<html />',
    })
    ...
    expect(result).toMatchObject({
      type: 'awaiting_user',
      reason: 'wireframe',
      wireframe: { relativeUrl: 'wireframe/wireframe.html', pageCount: 2, version: 'v1' },
    })
    expect(batches.at(-1)).toMatchObject({
      events: [
        {
          kind: 'wireframe/produced',
          payload: { relativeUrl: 'wireframe/wireframe.html', pageCount: 2, version: 'v1' },
        },
      ],
    })
    await expect(
      buildSessionTurnTools(context).write_wireframe.execute!({
  ...
  it('rejects malformed and over-broad tool arguments before execution', async () => {
    ...
    await expect(
      tools.write_wireframe.execute!({
        relativeUrl: 'wireframe/a.txt',
        pageCount: 1,
        version: 'v1',
        content: 'x',
      }),
