...
public class CreditServiceImpl
    extends ServiceImpl<CreditLedgerMapper, CreditLedger>
    implements CreditService
{
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void refundRun(
        String runId,
        AgentCompleteStatusEnum status,
        Integer filesWritten,
        ...
        log.info(
            "退款成功，runId: {}, status: {}, settleAmount: {}, refundAmount: {}",
            runId,
            status,
            update.getSettleAmount(),
            refundAmount
        );
    }
    ...
    public void recharge(Long userId, int credits) {
        ...
        log.info(
            "积分充值成功，userId: {}, amount: {}, balance: {}",
            userId,
            credits,
            balanceOf(user) + credits
        );
    }
    ...
    public CreditLedger getByRunId(String runId) {
        ...
        return this.getOne(
            new com.mybatisflex.core.query.QueryWrapper().eq(CreditLedger::getRunId, runId)
        );
    }
    ...
    private CreditLedger getFrozenOrNull(String runId) {
        ...
        CreditLedgerStatusEnum statusEnum = CreditLedgerStatusEnum.getEnumByValue(
            ledger.getStatus()
        );
    ...
    private int calcInterruptedSettleAmount(int frozen, Integer milestoneCount) {
        AgentProperties.Credit credit = agentProperties.getCredit();
        double ratio =
            milestoneCount != null && milestoneCount >= REVIEW_MILESTONE_THRESHOLD
