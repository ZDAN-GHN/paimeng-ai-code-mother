...
public class GenerationRunServiceImpl
    extends ServiceImpl<GenerationRunMapper, GenerationRun>
    implements GenerationRunService
{
    ...
    public RunVO createRun(RunCreateRequest request) {
        ...
        synchronized (lockFor(request.getAppId())) {
            ...
            if (active != null) {
                log.warn(
                    "同 app 存在进行中的 run，拒绝新建，appId: {}, activeRunId: {}, newRunId: {}",
                    request.getAppId(),
                    active.getRunId(),
                    request.getRunId()
                );
    ...
    public RunVO updateRun(String runId, RunUpdateRequest request) {
        ...
        if (StrUtil.isNotBlank(phase)) {
            ThrowUtils.throwIf(
                GenerationRunPhaseEnum.getEnumByValue(phase) == null,
        ...
        if (
            StrUtil.isNotBlank(update.getPhase()) &&
            GenerationRunPhaseEnum.isTerminal(
                GenerationRunPhaseEnum.getEnumByValue(update.getPhase())
            ) &&
    ...
    public void completeRun(String runId, AgentCompleteRequest request) {
        ...
        if (
            ledger != null && !CreditLedgerStatusEnum.FROZEN.getValue().equals(ledger.getStatus())
        ) {
            log.info(
                "台账已终态（{}），迟到/重复回调幂等跳过，runId: {}",
                ledger.getStatus(),
                runId
            );
            return;
        ...
        AgentCompleteStatusEnum status = AgentCompleteStatusEnum.getEnumByValue(
            request.getStatus()
        );

        GenerationRun run = this.getById(runId);
        if (run != null) {
            GenerationRunPhaseEnum currentPhase = GenerationRunPhaseEnum.getEnumByValue(
                run.getPhase()
            );
            if (
                GenerationRunPhaseEnum.isTerminal(currentPhase) &&
                terminalPhaseOf(status) != currentPhase
            ) {
                log.warn(
                    "run 已终态（{}）与回调 status（{}）不一致，拒绝处理，runId: {}",
                    currentPhase.getValue(),
                    request.getStatus(),
                    runId
                );
                return;
        ...
        if (CollUtil.isNotEmpty(request.getMessages())) {
            for (AgentCompleteRequest.Message message : request.getMessages()) {
                ThrowUtils.throwIf(
                    ChatHistoryMessageTypeEnum.getEnumByValue(message.getMessageType()) == null,
                    ErrorCode.PARAMS_ERROR,
                    "messageType 仅接受 user/ai"
                );
                ...
                if (
                    status == AgentCompleteStatusEnum.ABORTED &&
        ...
        if (status == AgentCompleteStatusEnum.SUCCESS) {
            ...
            String errorCode = normalizeFailureCode(
                StrUtil.blankToDefault(request.getErrorCode(), "unknown")
            );
            String errorMessage = StrUtil.blankToDefault(request.getErrorMessage(), "生成失败");
            log.warn(
                "Agent 生成失败，runId: {}，errorCode: {}，message: {}",
                runId,
                errorCode,
                errorMessage
            );
    ...
    public CreditFreezeVO freezeCredit(String runId, CreditFreezeRequest request) {
        ...
        if (StrUtil.isNotBlank(run.getCreditLedgerRef())) {
            ...
            if (ledger != null) {
                log.info(
                    "run 已冻结，幂等返回既有台账，runId: {}, ledgerId: {}",
                    runId,
                    ledger.getId()
                );
        ...
        ThrowUtils.throwIf(
            !GenerationRunPhaseEnum.WIREFRAME_CONFIRMED.getValue().equals(run.getPhase()),
            ErrorCode.FORBIDDEN_ERROR,
            "当前阶段（" + run.getPhase() + "）不能冻结积分，请先确认线框"
        );
        CreditFreezeVO vo = creditService.freeze(
            runId,
            run.getAppId(),
            run.getUserId(),
    ...
    public boolean acquireWireframeDailyQuota(Long userId) {
        ...
        rateLimiter.trySetRate(
            RateType.OVERALL,
            agentProperties.getWireframeDailyLimit(),
    ...
    private void validateComplete(AgentCompleteRequest request) {
        ThrowUtils.throwIf(request == null, ErrorCode.PARAMS_ERROR, "请求不能为空");
        ThrowUtils.throwIf(
            request.getAppId() == null || request.getAppId() <= 0,
            ErrorCode.PARAMS_ERROR,
            "appId 不能为空"
        );
        ThrowUtils.throwIf(
        ...
        ThrowUtils.throwIf(
            status == null,
            ErrorCode.PARAMS_ERROR,
            "status 仅接受 success/failed/aborted"
        );

        ThrowUtils.throwIf(
            status == AgentCompleteStatusEnum.SUCCESS &&
                StrUtil.isBlank(request.getWorkspacePath()),
            ErrorCode.PARAMS_ERROR,
            "workspacePath 不能为空"
        );
    }
    ...
    private void markRunTerminal(String runId, GenerationRunPhaseEnum phase) {
        ...
        if (
            GenerationRunPhaseEnum.isTerminal(
                GenerationRunPhaseEnum.getEnumByValue(existing.getPhase())
            )
        ) {
            return;
        }
    ...
    private void validateCreate(RunCreateRequest request) {
        ThrowUtils.throwIf(request == null, ErrorCode.PARAMS_ERROR, "请求不能为空");
        ThrowUtils.throwIf(
            StrUtil.isBlank(request.getRunId()),
            ErrorCode.PARAMS_ERROR,
            "runId 不能为空"
        );
        ThrowUtils.throwIf(
        ...
        ThrowUtils.throwIf(
            GenerationRunPhaseEnum.getEnumByValue(request.getPhase()) == null,
            ErrorCode.PARAMS_ERROR,
            "phase 非法"
        );
        validateJsonField("context", request.getContext());
    ...
    private boolean hasChanges(GenerationRun existing, GenerationRun update) {
        return (
            !Objects.equals(existing.getPhase(), update.getPhase()) ||
