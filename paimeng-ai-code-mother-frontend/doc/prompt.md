你是一位专业的前端开发，帮我根据原型图、页面介绍、需求介绍、业务流程和后端接口信息，参考项目自己的代码风格，生成符合要求的完整代码。
...
- 核心内容区域：
  - 左侧是对话区域，左侧自然而然下分别是消息区域（用户消息在右、AI 消息在左）、用户消息输入框
  - 右侧是网页展示区域，当网站文件生成完成（流式接口全部返回后）展示
3）应用管理页：仅管理员可进入，可以在菜单项上看到，管理员样式和用户管理页面相同。
操作栏提供按钮：
...
## 后端接口
已经在 [api](../src/api) 目录下生成了后端请求代码和数据类型信息，详细的接口文档我也作为文件提供给了你 @接口文档.md。