...
class AgentSseAdapterTest {
    ...
    void nullDataEventsAreFiltered() {
        ...
        AgentSseAdapter adapter = new AgentSseAdapter(jsonHandler, simpleText);

        Flux<AgentClient.SseEvent> input = Flux.just(
                new AgentClient.SseEvent(null, null),
                new AgentClient.SseEvent("error", "{\"message\":\"boom\"}"),
                new AgentClient.SseEvent(null, "正常文本"),
                new AgentClient.SseEvent(null, null),
                new AgentClient.SseEvent(null, "后续文本")
        );
    ...
    void errorEventsAreFiltered() {
        ...
        Flux<AgentClient.SseEvent> input = Flux.just(
                new AgentClient.SseEvent("error", "{\"message\":\"拒绝\"}"),
                new AgentClient.SseEvent(null, "ai 文本")
        );
        List<String> output = adapter.adapt(input, CodeGenTypeEnum.HTML, mock(ChatHistoryService.class), 1L, null)
                .collectList().block();
    ...
    void vueRoutesToJsonHandler() {
        ...
        Flux<AgentClient.SseEvent> input = Flux.just(
                new AgentClient.SseEvent(null, "{\"type\":\"ai_response\",\"data\":\"ok\"}")
        );
